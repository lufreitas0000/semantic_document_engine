...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[
        ...
        \node[infra, left=of interfaces] (postgres) {\textbf{PostgreSQL} \\ \footnotesize (Repository Adapter)};
        \node[infra, right=of interfaces] (fastapi) {\textbf{FastAPI} \\ \footnotesize (Web Adapter)};

        % FIX 1: Explicitly push Celery higher to clear the background box label
        \node[infra, above=2.5cm of interfaces] (celery) {\textbf{Celery} \\ \footnotesize (Worker Adapter)};

        % Background Box for Core
        \begin{pgfonlayer}{background}
            \node[draw=LightPink, fill=LightPink!5, dashed, rounded corners, fit=(domain)(interfaces), inner sep=0.5cm, label={[DarkSlate, font=\bfseries]north:Isolated System Locus}] (corebox) {};
        \end{pgfonlayer}

        % Dependency Arrows
        \draw[arrow] (interfaces) -- (domain) node[midway, right, font=\small] {uses};
        \draw[arrow] (postgres) -- (interfaces) node[midway, above, font=\small] {implements};
        \draw[arrow] (fastapi) -- (interfaces) node[midway, above, font=\small] {uses};
        \draw[arrow] (celery) -- (interfaces) node[midway, right, font=\small] {uses};
